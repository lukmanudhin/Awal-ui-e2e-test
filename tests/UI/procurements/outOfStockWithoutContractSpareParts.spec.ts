...
test.describe('Material Indent and Material Issue For Out of Stock Spare Parts With out Contract End-to-End Scenarios', () => {
    ...
    test('Verify an out of stock material with no contract is procured through a vendor quotation and issued after put away', async ({ salesEnquiryAPI, stockViewAPI, putAwayPage, grnEntryPage, materialIndentRequestAPI, procurementPage, prRequestPage, modules, materialIndentRequestPage, vendorRegistrationPage }) => {
        ...
        await test.step('Approve the Material Indent Request as the manager', async () => {
            ...
            await materialIndentRequestPage.validateMIRDetails(materialIndentRequestId, MIRDetails.priority, requestedBy);
            await materialIndentRequestPage.validateMaterialInformationTable(MIRDetails);
        ...
        await test.step('Verify the material cannot be issued while it is out of stock', async () => {
            ...
            await materialIndentRequestPage.clickViewIcon();
            await materialIndentRequestPage.validateMIRDetails(MIRDetails.requisitionType, requestedBy, 'Sales');
            await materialIndentRequestPage.validateMaterialInformationTable(MIRDetails);
