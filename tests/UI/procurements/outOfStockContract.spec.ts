...
test.describe('Material Indent and Material Issue End-to-End Scenarios', () => {
    ...
    test('Verify Material Indent Request is successfully created, approved by manager, and material is issued with Random Quantity', async ({ materialIndentRequestAPI, salesEnquiryAPI, stockViewAPI, putAwayPage, grnEntryPage, procurementPage, prRequestPage, modules, materialIndentRequestPage }) => {
        ...
        await test.step('Verify cancel purchase order eligiblity status', async () => {
            await modules.goToModule({ nestedSubModule: 'Purchase Order' });
            await materialIndentRequestPage.search(poNumber);
            await expect(materialIndentRequestPage.status('PO Approved'), 'PO status does not match').toBeVisible();
            await materialIndentRequestPage.clickCancelPO();
            await expect(materialIndentRequestPage.status('Yes'), 'PO cancel eligibility status does not match').toBeVisible();
            await materialIndentRequestPage.validateCancelPOButtonToBeEnabled();
        });
        ...
        await test.step('Complete QC on the received quantity using Random Quantity', async () => {
