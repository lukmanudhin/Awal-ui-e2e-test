...
test.describe('Request For Info - Subcontractor Service End-to-End Scenarios', () => {
    ...
    test('Verify a subcontractor service in a cost estimation is procured through a vendor quote comparison and the awarded vendor price reflects in the cost distribution', async ({ salesEnquiryPage, ppjoPage, page, modules, requestNormalPage, costEstimationPage, procurementPage }) => {
        ...
        await test.step('Send the subcontractor service to procurement', async () => {
            await costEstimationPage.goToTab('Procurement Request');
            await costEstimationPage.addSubcontractorServiceAndSendToProcurement(MIRDetails.subcontractorService);
            ...
            await expect(costEstimationPage.serviceProcurementStatus, "Service procurement status is not Pending").toContainText('Pending');
        });
        ...
        await test.step('Create a vendor quotation for an existing vendor and a new temp vendor', async () => {
            ...
            await procurementPage.selectRequestType('Subcontractor');
            ...
            await procurementPage.validateVendorQuotationServiceTable(MIRDetails.subcontractorService);
        ...
        await test.step('Enter the quote received from the temp vendor', async () => {
            ...
            await procurementPage.selectRequestType('Subcontractor');
            MIRDetails.unitPrice = '250';
            await procurementPage.openVendorQuote(enquiryId, MIRDetails.tempVendorName);
            await procurementPage.enterSubcontractorQuoteDetails(MIRDetails);
        ...
        await test.step('Enter the lower quote received from the existing vendor', async () => {
            // Coming back from a quote resets the list filter to Material.
            await procurementPage.selectRequestType('Subcontractor');
            MIRDetails.unitPrice = '200';
        ...
        await test.step('Award the quotation to the existing vendor and submit for estimation', async () => {
            await procurementPage.goToTab('Quotes Received');
            await procurementPage.selectRequestType('Subcontractor');
        ...
        await test.step('Reopen the cost estimation and include the awarded service price', async () => {
            await modules.goToModule({ module: 'Estimation', subModule: 'Request (Normal)' });
            ...
            await costEstimationPage.clickAddEstimation();
            await costEstimationPage.goToTab('Procurement Request');
            await expect(costEstimationPage.serviceProcurementStatus, "Service procurement status is not Completed").toContainText('Completed');
            await costEstimationPage.includeServicePrice(MIRDetails.vendorQuotationVendor, MIRDetails.unitPrice);
            await expect(costEstimationPage.successMessage('Procurement service price updated successfully'), "Procurement service price updated successfully message does not match").toContainText('Procurement service price updated successfully');
        });

        await test.step('Verify the awarded service price reflects in the cost distribution', async () => {
            await costEstimationPage.goBackFromServiceQuoteComparison();
            const serviceCost = (await costEstimationPage.serviceProcurementCost.innerText()).trim();
            expect(serviceCost, 'Procurement Service Cost is empty after including the price').toBe(`${MIRDetails.unitPrice}.000`);
            console.log(`Procurement service cost after including the awarded price: ${serviceCost}`);
            await costEstimationPage.goToTab('Cost Distribution');
            await expect(page.getByRole('table').first(), `Cost Distribution does not reflect the procured service cost: ${serviceCost}`).toContainText(serviceCost);
        });
    });
});