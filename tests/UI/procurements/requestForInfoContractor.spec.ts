...
test.describe('Request For Info - Subcontractor Service End-to-End Scenarios', () => {
    ...
    test('Verify a subcontractor service in a cost estimation is procured through a vendor quote comparison and the awarded vendor price reflects in the cost distribution', async ({ salesEnquiryPage, ppjoPage, page, modules, requestNormalPage, costEstimationPage, procurementPage }) => {
        ...
        await test.step('Send the subcontractor service to procurement', async () => {
            ...
            await expect(costEstimationPage.status('Pending'), "Service procurement status is not Pending").toBeVisible();
        });
        ...
        await test.step('Reopen the cost estimation and include the awarded service price', async () => {
            ...
            await costEstimationPage.clickAddEstimation();
            await costEstimationPage.goToTab('Procurement Request');
            await expect(costEstimationPage.status('Completed'), "Service procurement status is not Completed").toBeVisible();
