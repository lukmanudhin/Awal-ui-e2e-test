...
test.describe.serial('Verify Sales Order Manager Rejects the Checklist', () => {
    ...
    test('Verify Sales Order Manager rejects the checklist', async ({ page, modules, salesEnquiryPage, salesOrderManagerPage, quotationManagerPage, ppjoPage, invoiceRequestPage, requestNormalPage, costEstimationPage, requestApprovalPage }) => {
        ...
        await test.step('Verify Sales Checklist Rejected by Manager status is reflected in sales enquiry', async () => {
            await modules.goToModule({ module: 'Sales', subModule: 'Sales Enquiry' });
            await salesEnquiryPage.search(enquiryId);
            await expect(salesEnquiryPage.enquiryStatus, "Sales order rejected status does not match in sales enquiry").toContainText('Sales Checklist Rejected by Manager');
        });
    });
});