...
test.describe.serial('Verify Quotation Under Consideration flow of Sales Enquiry (Request Normal)', () => {
    ...
    test('Verify customer rejects quotation and the status is reflected in quotation', async ({ page, modules, salesEnquiryPage, ppjoPage, requestNormalPage, costEstimationPage, requestApprovalPage, quotationManagerPage }) => {
        ...
        await test.step('Quotation: Verify Quotation Under Consideration flow of Sales Enquiry (Request Normal)', async () => {
            ...
            await expect(quotationManagerPage.successMessage('Under consideration submitted successfully'), "Under consideration submitted successfully message does not match").toHaveText('Under consideration submitted successfully');
            await quotationManagerPage.search(enquiryId);
            await expect(quotationManagerPage.quotationStatus, 'Quotation status does not match').toHaveText('Quotation Under Consideration');
        });

        await test.step('Verify Quotation Under Consideration status is reflected in sales enquiry', async () => {
            await modules.goToModule({ subModule: 'Sales Enquiry' });
            await salesEnquiryPage.search(enquiryId);
            await expect(salesEnquiryPage.enquiryStatus, "Customer Hold status does not match in sales enquiry").toHaveText('Quotation Under Consideration');
        });
    });
});