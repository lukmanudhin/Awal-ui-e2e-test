...
test.describe.serial('Verify E2E Estimation Rejection flow of Sales Enquiry (Request Normal)', () => {
    ...
    test('Verify that the cost estimation is rejected with rejected status reflected in history', async ({ page, modules, salesEnquiryPage, requestApprovalPage, costEstimationPage, ppjoPage, requestNormalPage }) => {
        ...
        await test.step('Verify that the cost estimation is rejected with rejected status reflected in history', async () => {
            ...
            await costEstimationPage.goToTab('Summary');
            await costEstimationPage.goToTab('Price Indication Slip');
            await requestApprovalPage.rejectEstimationAndValidateAPI(200);
            //bug need to update success message
            await expect(requestApprovalPage.successMessage('Estimation rejected successfully'), "Estimation rejected successfully message does not match").toContainText('Estimation rejected successfully');
            await expect(requestApprovalPage.rejectStatus, 'Rejected status is not visible').toBeVisible();
            await costEstimationPage.resubmitToEstimator();
            await expect(costEstimationPage.successMessage('Resubmitted to estimator successfully'), "Resubmitted to estimator successfully message does not match").toContainText('Resubmitted to estimator successfully');
            await modules.goToModule({ subModule: 'Request Approval' });
