...
// import { createEnquiryData } from "../../../testData/salesEnquiryData";
// import { addBOQData } from "../../../testData/addBoqData";
// import { Utils } from "../../../utils/utils";

// test.describe('Request Estimation displayed in estimation list', () => {
//     test.setTimeout(550000);
//     let enquiryId: string;
//     test.beforeEach('Login and Create Sales Enquiry', async ({ salesEnquiryPage, productsPage, loginPage, page, homePage, ppjoPage }) => {
...
//             await salesEnquiryPage.clickCreatePPJO(createEnquiryData.customerName);
...
//             // await salesEnquiryPage.validateCustomerPPJOColumn(createEnquiryData.customerName, ['Artwork', 'AutoCAD', 'Estimation', 'Procurement']);
//         });
//     });

//     test.afterEach('Delete Sales Enquiry', async ({ salesEnquiryPage, page, modules }) => {
...
//     test('Verify requested estimation is displayed in estimation list', async ({ salesOrderManagerPage, invoiceRequestPage, quotationManagerPage, ppjoPage, requestApprovalPage, salesEnquiryPage, requestNormalPage, page, modules, costEstimationPage }) => {
//         await test.step('Verify estimation request is available in Request Normal', async () => {
...
//         });
//         await test.step('Generate cost estimation and validate request attachments', async () => {
//             await requestNormalPage.clickGenerateCostEstimation(createEnquiryData.customerName);
...
//         await test.step('Create BOQ and generate BOQ details', async () => {
...
//         await test.step('BOQ Details: add consumables, BOM, and update BOL', async () => {
...
//         await test.step('Save BOQ summary with variation', async () => {
...
//         await test.step('Generate packing cost and add packing BOM', async () => {
...
//         await test.step('Add packing labour and save packing summary', async () => {
...
//         await test.step('Cost Distribution: save sample and distribution costs', async () => {
...
//         await test.step('Summary: calculate incentive summary', async () => {
//             await costEstimationPage.goToTab('Summary');
...
//             await costEstimationPage.editSummary(createEnquiryData);
...
//         await test.step('Price Indication Slip: save Version 1 and submit for approval', async () => {
...
//         await test.step('Verify Request Normal status is Pending For Approval', async () => {
//             await costEstimationPage.goBackToEstimationListPage();
//             await salesEnquiryPage.search(enquiryId);
//             await expect(requestNormalPage.estimationStatus('Pending For Approval'), "Pending For Approval status does not match").toHaveText('Pending For Approval');
//         });

//         await test.step('Approve cost estimation and submit it to Sales', async () => {
...
//             await expect(requestApprovalPage.successMessage('Price indication slip saved successfully'), "Price indication slip saved successfully message does not match").toContainText('Price indication slip saved successfully');
...
//         await test.step('Verify Sales Enquiry moves to Quotation Pending and generate quotation', async () => {
//             await modules.goToModule({ module: 'Sales', subModule: 'Sales Enquiry' });
//             await salesEnquiryPage.search(enquiryId);
...
//         await test.step('Quotation Manager: Manager approves quotation', async () => {
...
//         await test.step('Quotation: Send quotation to customer and request advance invoice', async () => {
...
//             await quotationManagerPage.enterCustomerApprovalAttachment();
//             await quotationManagerPage.validateQuotationApprovalAPI(201);
//             await expect(quotationManagerPage.customerApprovalAttachment, "Customer approval attachment is not visible").toHaveCount(1);
//             await quotationManagerPage.sendAdvanceInvoiceAndValidateAPI(201);

//         });

//         await test.step('Invoice Request: Create advance invoice from Invoice Request', async () => {
...
//         await test.step('Invoice Register: Manager approves and acknowledges invoice register', async () => {
//             await modules.goToModule({ nestedSubModule: 'Invoice Register (Manager)' });
//             await invoiceRequestPage.search(createEnquiryData.customerName);
...
//         await test.step('Generate sales order checklist from customer approved quotation', async () => {
...
//         await test.step('Manager approves sales order checklist', async () => {
...
//         await test.step('Submit sales order for approval', async () => {
...
//         await test.step('Manager approves sales order', async () => {
//             await modules.goToModule({ subModule: 'Sales Order (Manager)' });
//             await salesOrderManagerPage.search(enquiryId);
...
//         await test.step('Verify final Sales Enquiry quotation and sales order tabs', async () => {
